...
**页面导航必须使用相对路径**，以确保应用可以部署在任意子路径下（如 `/mobo/`）。

| 场景 | 正确写法 | 错误写法 |
|------|----------|----------|
| `<a>` 标签 | `href="./registry.html"` | `href="/registry.html"` |
| JS 跳转 | `window.location.href = './today.html?id=1'` | `window.location.href = '/today.html?id=1'` |

**原因：** 绝对路径（以 `/` 开头）会从网站根目录解析，在子路径部署时会导致 404。

**例外：** API 调用使用 `API_BASE` 配置，不受此规则约束。
